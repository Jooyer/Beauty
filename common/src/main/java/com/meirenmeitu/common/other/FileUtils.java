...
public class FileUtils {
    ...
    public static String getCachePath() {
        if (isSdCardExist()) {
            File dir = RxRetrofit.Companion.getInstance().getMContext()
                    .getExternalCacheDir();
            return null == dir ? RxRetrofit.Companion.getInstance().getMContext()
                    .getCacheDir().getAbsolutePath() : dir.getAbsolutePath();
        } else {
            return RxRetrofit.Companion.getInstance().getMContext()
                    .getCacheDir()
                    .getAbsolutePath();
        }
    }
    ...
    public static List<File> getTxtFiles(String filePath, int layer) {
        final List txtFiles = new ArrayList(); // @FIXME lyz 3/19 14:30
        File file = new File(filePath);
