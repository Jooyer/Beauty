package com.meirenmeitu.library.refresh.header;
...
public class DefaultHeader extends BaseHeaderView {
    ...
    public void handleDrag(float dragY) {
        ...
        if(dragY <= 0){//回到初始位置
            status = HEADER_DRAG;
            ivHeaderTip.setBackgroundResource(R.mipmap.ic_refresh_pull_down);
            ivHeaderTip.setVisibility(VISIBLE);
            tvHeaderTip.setText(mContext.getResources().getString(R.string.lib_refresh_drag));
        ...
        if (status == HEADER_DRAG) {//开始拖动
            if(dragY >= childHeight) {//一旦超过刷新头高度
                ...
                ivHeaderTip.setVisibility(VISIBLE);
                startRotateAnimDown();
                tvHeaderTip.setText(mContext.getResources().getString(R.string.lib_refresh_release));
                pbRefreshing.setVisibility(GONE);
            }
        }
    ...
    public boolean checkRefresh() {
        if ((status == HEADER_RELEASE || status == HEADER_REFRESHING) && totalOffset>=childHeight) {
            ...
            tvHeaderTip.setText(mContext.getResources().getString(R.string.lib_refreshing));
    ...
    public void refreshCompleted() {
        ...
        ivHeaderTip.clearAnimation();
        ivHeaderTip.setBackgroundResource(R.mipmap.ic_refresh_data_completed);
        ivHeaderTip.setVisibility(VISIBLE);
        tvHeaderTip.setText(mContext.getResources().getString(R.string.lib_refresh_completed));
        pbRefreshing.setVisibility(GONE);
    }
